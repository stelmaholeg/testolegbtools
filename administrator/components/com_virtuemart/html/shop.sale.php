...
if (defined('VM_ALLOW_EXTENDED_CLASSES') && defined('VM_THEMEPATH') && VM_ALLOW_EXTENDED_CLASSES && file_exists(VM_THEMEPATH.'user_class/shop_sale_queries.php')) {
	// Load the theme-user_class shop_browse_queries.php
	require_once( VM_THEMEPATH.'user_class/shop_sale_queries.php' );
} else {
	require_once( PAGEPATH. "shop_sale_queries.php" );
}
...
elseif( $num_rows == 1 && ( !empty($keyword) || !empty($keyword1) ) ) {
	...
	elseif( $keyword ) {
		...
		$mainframe->setPageTitle('Распродажа' );#
		$browsepage_lbl = 'Распродажа';
		$tpl->set( 'browsepage_lbl', $browsepage_lbl );
	...
	if ( $num_rows > 1 && @$_REQUEST['output'] != "pdf") {
		if ( $num_rows > 5 ) { // simplified logic
			$search_string = $mm_action_url."index.php?option=com_virtuemart&amp;page=$modulename.sale";
