<?php
/**
 * @version		1.6.0.72 helpers/importer.php
	...
	static function import($xml, $filever, $xmlrpc = false)
	{
		...
		if ($import_users)
		{
			foreach($xml->xpath("user") as $record)
			{
				...
				$query = 'SELECT id, name'
					. ' FROM #__users'
					. ' WHERE'. ((($attributes->mapkeystotext == 'false') || ($keep_id == 1))
						? ' id = '.$id
				...
				if (!$user || ($import_users == 2))
				{
					...
					if ($table->save($data))
					{
						if (!$user && (($attributes->mapkeystotext == 'false') || ($keep_id == 1)))
						{
		...
		foreach($xml->xpath("content") as $record)
		{
			...
			{ 
				...
				if ($table->save($data))
				{
					if (!$content && (($attributes->mapkeystotext == 'false') || ($keep_id == 1)))
					{
