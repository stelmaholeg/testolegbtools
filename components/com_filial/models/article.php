...
/**
 * Filial Component Article Model
 *
 * @package		Joomla.Site
 * @subpackage	com_filial
 * @since 1.5
 */
class FilialModelArticle extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $_context = 'com_filial.article';
	...
	public function &getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk])) {
			...
			try {
				...
				$query->join('LEFT', '#__filial_rating AS v ON a.id = v.filial_id');
				...
				$subquery .= 'WHERE parent.extension = ' . $db->quote('com_filial');
				...
				if (empty($data)) {
					return JError::raiseError(404, JText::_('COM_FILIAL_ERROR_ARTICLE_NOT_FOUND'));
				}
				...
				if (!$user->get('guest')) {
					$userId	= $user->get('id');
					$asset	= 'com_filial.article.'.$data->id;
    ...
    public function storeVote($pk = 0, $rate = 0)
    {
        if ( $rate >= 1 && $rate <= 5 && $pk > 0 )
        {
            ...
            $db->setQuery(
                    'SELECT *' .
                    ' FROM #__filial_rating' .
                    ' WHERE filial_id = '.(int) $pk
            );
            ...
            if (!$rating)
            {
                ...
                $db->setQuery(
                        'INSERT INTO #__filial_rating ( filial_id, lastip, rating_sum, rating_count )' .
        ...
        JError::raiseWarning( 'SOME_ERROR_CODE', JText::sprintf('COM_FILIAL_INVALID_RATING', $rate), "JModelArticle::storeVote($rate)");
        return false;
    }
}
