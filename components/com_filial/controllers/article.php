 ...
 */
class FilialControllerArticle extends JControllerForm
{
	/**
	 * @since	1.6
	 */
	...
	protected function allowAdd($data = array())
	{
		...
		if ($categoryId) {
			...
			$allow	= $user->authorise('core.create', 'com_filial.category.'.$categoryId);
		}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$user		= JFactory::getUser();
		$userId		= $user->get('id');
		$asset		= 'com_filial.article.'.$recordId;
...
//		}
	...
	protected function postSaveHook(JModel &$model, $validData)
	{
		...
		if ($task == 'save') {
			$this->setRedirect(JRoute::_('index.php?option=com_filial&view=category&id='.$validData['catid'], false));
		}
	}
	...
	public function save($key = null, $urlVar = 'a_id')
	{
		...
		require_once JPATH_ADMINISTRATOR.'/components/com_filial/helpers/filial.php';
	...
	function vote()
	{
		...
		if ( $user_rating > -1 ) {
			...
			if ($model->storeVote($id, $user_rating)) {
				$this->setRedirect($url, JText::_('COM_FILIAL_ARTICLE_VOTE_SUCCESS'));
			} else {
				$this->setRedirect($url, JText::_('COM_FILIAL_ARTICLE_VOTE_FAILURE'));
			}
		}
	}
}
